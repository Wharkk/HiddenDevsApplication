...
retriveParts(game.Workspace.SpawnedParts);
game.Workspace.SpawnedParts.ChildAdded:connect(retriveParts)
game.Workspace.SpawnedParts.ChildRemoved:connect(RemoveBits)
...
function GetBaseSize()
	...
	if(Mesh ~= nil) then
		Size = Vector3.new((mySize.X*Mesh.Scale.X), (mySize.Y*Mesh.Scale.Y), (mySize.Z*Mesh.Scale.Z))
	else
		Size = mySize
	end
	return Size
...
function GetBasePosition()
	...
	local Mesh = Self:findFirstChild("Mesh")
	if(Mesh ~= nil) then
		Pos = Vector3.new((myPos.X+Mesh.Offset.X), (myPos.Y+Mesh.Offset.Y), (myPos.Z+Mesh.Offset.Z))
	else
		Pos = myPos
	end
	return Pos
	...
	if(part~= nil) then
		...
		and(PartPos.Y-(PartSize.Y/2) < MyPos.Y+(Area.Y/2))) then
			...
			and(PartPos.X-(PartSize.X/2) < MyPos.X+(Area.X/2))) then 
				...
				and(PartPos.Z-(PartSize.Z/2) < MyPos.Z+(Area.Z/2))) then
					isIA = true
				end
			end
		end
